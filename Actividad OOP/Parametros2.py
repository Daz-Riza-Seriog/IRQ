...
class set_Reactor2:
    ...
    F_A0 = 2.4936002609389596  # Valor dado en el trabajo anterior derivado de la meta a producir mol/s
...
class set_Rate2:
    ...
    k_eq_1 = k_eq_est1 * np.exp(
        (H_rxn_1 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # Constante de equilibrio 1
    k_eq_2 = k_eq_est2 * np.exp(
        (H_rxn_2 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # Constante de equilibrio 2
    k_eq_3 = k_eq_est3 * np.exp(
        (H_rxn_3 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # Constante de equilibrio 3
    ...
    K_ads_h2 = K_ads_h2_300 * np.exp(
        (H_ads_h2_300 / R_1) * ((1 / Reactor2.T_0) - (1 / Reactor2.T_0)))  # Constante de adsorcion H2
    K_ads_co2 = K_ads_co2_300 * np.exp(
        (H_ads_co2_300 / R_1) * ((1 / Reactor2.T_0) - (1 / Reactor2.T_0)))  # Constante de adsorcion CO2
    ...
    k_vel_1 = k_vel_1_ref * np.exp((E_a_1 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # CH3OH [mol/s*bar^2k*kgcat]
    k_vel_2 = k_vel_2_ref * np.exp((E_a_2 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # RWGS [mol/s*bar^1.5k*kgcat]
    k_vel_3 = k_vel_3_ref * np.exp(
        (E_a_3 / R_1) * ((1 / T_est) - (1 / Reactor2.T_0)))  # Methanation [mol/s*bar^1k*kgcat]

    # Inhibition Parameter Initial
    inh_0 = (1 + (K_ads_co2 * Reactor2.p_A0) + np.sqrt(K_ads_h2 * Reactor2.p_B0)) ** 2

Rate = set_Rate2()

class Estequiometria2:
    __metaclass__ = Reactor2

    # Initial Concentrations
    C_A0 = Reactor2.C_A0
    C_B0 = Reactor2.C_B0
    C_C0 = Reactor2.F_C0 / Reactor2.vflow_0
    C_D0 = Reactor2.F_D0 / Reactor2.vflow_0
    C_E0 = Reactor2.F_E0 / Reactor2.vflow_0
    C_F0 = Reactor2.F_F0 / Reactor2.vflow_0
    ...
    sigma = (a_c + b_c - d_c - 1)
    Eps = sigma * Reactor2.y_co2_o


class set_initial_state_flux_drop2:
    __metaclass__ = Rate
    __metaclass__ = Reactor2
    x_0 = np.empty(10)  # allocate memory
    x_0[0] = Reactor2.F_A0  # molar flow rate of A
    x_0[1] = Reactor2.F_B0  # molar flow rate of B
