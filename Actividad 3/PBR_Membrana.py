...
def PBR_calc_f(W, x, Rate, Reactor):
    ...
    C_D = P_D / (Reactor.R * Reactor.T_0)  # Cocentración del H2O [mol/m3]

    # Difusión de salida H20
    R_C = Reactor.K * C_D  # Este es el termino que se resta #mol/kg cat*s
...
sol_PBR = solve_ivp(PBR_calc_f, [0, Reactor.V_tot_su], ini_state.x_0, args=(Rate, Reactor), t_eval=t_eval, rtol=1e-12,
                    atol=1e-12)
...
Cat_Sel_max = round(sol_PBR.t[S_max]-1.55, 4)
print("maximo selectividad 1 ´maximo grafica´", Sel_max)

# Find the position of maximum selectivity 2
S_max2 = np.asarray(sol_PBR.y[6]).argmax()
Sel_max2 = round(S_2[S_max2], 4)
Cat_Sel_max2 = round(sol_PBR.t[S_max2]-1.55, 4)
print("maximo selectividad 2 ´maximo grafica´", Sel_max2)

# Find the position of maximum selectivity 1
S_max3 = np.asarray(sol_PBR.y[6]).argmax()
Sel_max3 = round(S_3[S_max3], 4)
Cat_Sel_max3 = round(sol_PBR.t[S_max3]-1.55, 4)
print("maximo selectividad 3 ´maximo grafica´", Sel_max3)
...
Time_Yil_max_1=round(sol_PBR.t[Y_max_1]-1.55, 4)
...
class plot_results:
    ...
    plt.vlines(sol_PBR.t[S_max2]-1.55, 0, S_1[S_max2], label=f"Selectividad Maxima {Sel_max2},\nCatalizador {Cat_Sel_max2}",
               linestyle="dotted", colors="c")
    plt.xlabel("Catalizador $[Kg]$", labelpad=15, fontsize=13)
    plt.ylabel(r'$r_{selectividad}$', labelpad=8, fontsize=12)
    plt.legend()
    ...
    plt.vlines(sol_PBR.t[S_max3]-1.55, 0, S_3[S_max3], label=f"Selectividad Maxima {Sel_max},\nCatalizador {Cat_Sel_max}",
               linestyle="dotted", colors="c")
    plt.xlabel("Catalizador $[Kg]$", labelpad=15, fontsize=13)
    plt.ylabel(r'$r_{selectividad}$', labelpad=8, fontsize=12)
    plt.legend()
    ...
    plt.vlines(sol_PBR.t[Y_max_1]-1.55, 0, Y_1_1[Y_max_1],
               label=f"Rendimiento Maximo {Yil_max_1},\nCatalizador{Time_Yil_max_1}",
               linestyle="dotted", colors="c")
    plt.xlabel("Catalizador $[Kg]$", labelpad=15, fontsize=13)
    ...
    plt.plot(sol_PBR.t, sol_PBR.y[4], label=r"$F_{CO}$")
