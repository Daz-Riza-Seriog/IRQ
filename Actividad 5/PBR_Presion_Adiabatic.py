...
ini_state = Parametros.set_initial_state_flux_drop_adia
...
def PBR_calc_f(W, x, Rate, Reactor):
    ...
    # Qrem = (Rate.Ua / Reactor.rc) * (T - Ta) *0 # TODO corregir por que es rho_b
    ...
    f = [dFAdW, dFBdW, dFCdW, dFDdW, dFEdW, dPdW, dTdW, rA1, rA2, rA3]
...
class plot_results:
    ...
    axs[0, 0].set_title("PBR Adiabatic\nCatalizador vs Flujo")
    ...
    axs[0, 1].set_title("PBR Adiabatic\n Catalizador vs Caida Presion")
    ...
    axs[1, 0].set_title("PBR Adiabatic\n Temperatura Ta vs T")
    ...
    axs[1, 1].set_title("PBR Adiabatic\n Temperatura Ta vs T")
    axs[1, 1].plot(sol_PBR.t, sol_PBR.y[7], label=r"$r_1$")
    axs[1, 1].plot(sol_PBR.t, sol_PBR.y[8], label=r"$r_2$")
    axs[1, 1].plot(sol_PBR.t, sol_PBR.y[9], label=r"$r_3$")
