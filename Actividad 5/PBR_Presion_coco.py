...
def PBR_calc_f(W, x, Rate, Reactor):
    ...
    P = x[5]  # Presion
    Ta = x[6]  # Temperatura de Chaqueta
    T = x[7]  # Temperatura de Reactor
    rA1 = x[8]
    rA2 = x[9]
    rA3 = x[10]
    ...
    dFDdW = (2 * rA1) + rA2 + rA3
    dFEdW = rA2 - rA3
    dPdW = (var_1 * (Reactor.P0 / P) * (FT / Reactor.FT0))

    # Co-corriente
    Qgen = -rA1 * H_rxn_1 + rA2 * H_rxn_2 - rA3 * H_rxn_3
    Qrem = (Rate.Ua / ((1 - Reactor.f) * Reactor.rc)) * (T - Ta)
    Cpmix = (FA * Rate.Cp_CO2) + (FB * Rate.Cp_H2) + (FC * Rate.Cp_CH4) + (FD * Rate.Cp_H2O) + (FE * Rate.Cp_CO)
    dTadW = (Rate.Ua / ((1 - Reactor.f) * Reactor.rc)) * (T - Ta) / (
                Reactor.F_cool * Rate.Cp_cool)  # Co-Current Balance
    dTdW = (Qgen - Qrem) / Cpmix

    f = [dFAdW, dFBdW, dFCdW, dFDdW, dFEdW, dPdW, dTadW, dTdW, rA1, rA2, rA3]
...
class plot_results:
    ...
    axs[0, 0].set_title("PBR Co-Current\nCatalizador vs Flujo")
    ...
    axs[0, 1].set_title("PBR Co-Current\n Catalizador vs Caida Presion")
    ...
    axs[1, 0].set_title("PBR Co-Current\n Temperatura Ta vs T")
    ...
    axs[1, 1].set_title("PBR Co-Current\n Temperatura Ta vs T")
